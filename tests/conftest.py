...
@pytest.fixture(scope="session")
def mock_jwt_token():
    """A fixture that generates a valid-structured, signed JWT for testing."""
    payload = {
        "sub": "test-user",
        "exp": int(time.time()) + 3600  # Expires in 1 hour
    }
    # The key is irrelevant since the client doesn't verify the signature,
    # but the token must be properly encoded.
    token = jwt.encode(claims=payload, key="test-secret", algorithm="HS256")
    return token

@pytest.fixture
def mock_keycloak_requests(monkeypatch, mock_jwt_token):
    """
    Mocks both the OIDC discovery (requests.get) and token fetching (requests.post)
    for Keycloak authentication in the Client class.
    """
    class MockResponse:
    ...
    def mock_requests_call(method, url, *args, **kwargs):
        # Mock for the OIDC discovery GET request
        if method.lower() == 'get' and '.well-known/openid-configuration' in url:
            return MockResponse({
                "token_endpoint": "http://fake-keycloak/realms/test/protocol/openid-connect/token"
            })
        # Mock for the token fetching POST request
        if method.lower() == 'post' and '/protocol/openid-connect/token' in url:
            return MockResponse({
                "access_token": mock_jwt_token
            })
        # Fallback for any other unexpected request
        return MockResponse({"error": "Unexpected request"}, status_code=404)

    # Use a lambda to dynamically choose the mock based on the method
    monkeypatch.setattr(
        requests,
        "request",
        lambda method, url, **kwargs: mock_requests_call(method, url, **kwargs)
    )
